...
Script to build the FCK executables and pack all their dependencies in one folder.
"""
...
def build() -> None:
    ...
    if Path("dist/FCK").is_dir():
        shutil.rmtree("dist/FCK")
...
def finalize_build() -> None:
    ...
    for item in json.loads(Path("res/ext_resources.json").read_text(encoding="utf8")):
        for i in Path("res").glob(item):
            additional_items[i] = Path("dist/FCK") / "res" / i.relative_to(Path("res"))
    ...
    for item, dest in additional_items.items():
        ...
        logging.info(
            f"Copied {str(item)!r} to {str(dest.relative_to(Path('dist/FCK')))!r}."
        )
    ...
    zip_folder(Path("dist/FCK"), output_archive)
...
if __name__ == "__main__":
    try:
        ...
        shutil.rmtree("dist/FCK", ignore_errors=True)
